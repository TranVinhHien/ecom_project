...
from .agent import RAGAgent, OrderAgent, VoucherAgent
...
class HostAgent:
    ...
    async def agent_rag_tool(self, query: str, intent: str, tool_context: ToolContext):
        """
        Tool gọi RAGAgent để tìm kiếm và phân tích sản phẩm/chính sách
        
        Args:
            query: Câu hỏi của người dùng về sản phẩm/chính sách
    ...
    async def agent_order_tool(self, query: str, tool_context: ToolContext):
        """
        Tool gọi OrderAgent để tra cứu và quản lý đơn hàng
    ...
    async def agent_voucher_tool(self, query: str, tool_context: ToolContext):
        """
        Tool gọi VoucherAgent để tra cứu và tư vấn voucher
