...
func checkRole(role string) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if userPayload.Scope != role {
			ctx.AbortWithStatusJSON(403, assets_api.ResponseError(403, fmt.Sprintf("không có quyền truy cập, chức năng này chỉ dành cho %s", role)))
			return
		}
	}
}
func getShopID() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		_, exists := ctx.Get(authorizationPayload)
		if !exists {
			ctx.AbortWithStatusJSON(401, assets_api.ResponseError(401, "token không tồn tại"))
			return
		}
