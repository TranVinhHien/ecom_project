package assets_services

import (
	"fmt"
	"strings"
	"unicode"
)

func ConvertStringToFloat(input string) (float64, error) {
	// Loại bỏ các ký tự không phải số và dấu thập phân
	cleaned := ""
	for _, r := range input {
		if unicode.IsDigit(r) || r == '.' || r == '-' {
			cleaned += string(r)
		}
	}

	// Chuyển đổi chuỗi đã làm sạch thành float64
	var result float64
	_, err := fmt.Sscanf(cleaned, "%f", &result)
	if err != nil {
		return 0, err
	}
	return result, nil
}
func ConvertSliceToQuotedString(slice []string) string {
	// Trích xuất Product_sku_id từ slice

	// Chuyển thành chuỗi đúng định dạng
	productSkuIDString := strings.Join(slice, ",")
	return productSkuIDString
}

// vietnameseToASCII ánh xạ các ký tự tiếng Việt có dấu sang không dấu
var vietnameseToASCII = map[rune]rune{
	'à': 'a', 'á': 'a', 'ả': 'a', 'ã': 'a', 'ạ': 'a',
	'ă': 'a', 'ằ': 'a', 'ắ': 'a', 'ẳ': 'a', 'ẵ': 'a', 'ặ': 'a',
	'â': 'a', 'ầ': 'a', 'ấ': 'a', 'ẩ': 'a', 'ẫ': 'a', 'ậ': 'a',
	'đ': 'd',
	'è': 'e', 'é': 'e', 'ẻ': 'e', 'ẽ': 'e', 'ẹ': 'e',
	'ê': 'e', 'ề': 'e', 'ế': 'e', 'ể': 'e', 'ễ': 'e', 'ệ': 'e',
	'ì': 'i', 'í': 'i', 'ỉ': 'i', 'ĩ': 'i', 'ị': 'i',
	'ò': 'o', 'ó': 'o', 'ỏ': 'o', 'õ': 'o', 'ọ': 'o',
	'ô': 'o', 'ồ': 'o', 'ố': 'o', 'ổ': 'o', 'ỗ': 'o', 'ộ': 'o',
	'ơ': 'o', 'ờ': 'o', 'ớ': 'o', 'ở': 'o', 'ỡ': 'o', 'ợ': 'o',
	'ù': 'u', 'ú': 'u', 'ủ': 'u', 'ũ': 'u', 'ụ': 'u',
	'ư': 'u', 'ừ': 'u', 'ứ': 'u', 'ử': 'u', 'ữ': 'u', 'ự': 'u',
	'ỳ': 'y', 'ý': 'y', 'ỷ': 'y', 'ỹ': 'y', 'ỵ': 'y',
	'À': 'A', 'Á': 'A', 'Ả': 'A', 'Ã': 'A', 'Ạ': 'A',
	'Ă': 'A', 'Ằ': 'A', 'Ắ': 'A', 'Ẳ': 'A', 'Ẵ': 'A', 'Ặ': 'A',
	'Â': 'A', 'Ầ': 'A', 'Ấ': 'A', 'Ẩ': 'A', 'Ẫ': 'A', 'Ậ': 'A',
	'Đ': 'D',
	'È': 'E', 'É': 'E', 'Ẻ': 'E', 'Ẽ': 'E', 'Ẹ': 'E',
	'Ê': 'E', 'Ề': 'E', 'Ế': 'E', 'Ể': 'E', 'Ễ': 'E', 'Ệ': 'E',
	'Ì': 'I', 'Í': 'I', 'Ỉ': 'I', 'Ĩ': 'I', 'Ị': 'I',
	'Ò': 'O', 'Ó': 'O', 'Ỏ': 'O', 'Õ': 'O', 'Ọ': 'O',
	'Ô': 'O', 'Ồ': 'O', 'Ố': 'O', 'Ổ': 'O', 'Ỗ': 'O', 'Ộ': 'O',
	'Ơ': 'O', 'Ờ': 'O', 'Ớ': 'O', 'Ở': 'O', 'Ỡ': 'O', 'Ợ': 'O',
	'Ù': 'U', 'Ú': 'U', 'Ủ': 'U', 'Ũ': 'U', 'Ụ': 'U',
	'Ư': 'U', 'Ừ': 'U', 'Ứ': 'U', 'Ử': 'U', 'Ữ': 'U', 'Ự': 'U',
	'Ỳ': 'Y', 'Ý': 'Y', 'Ỷ': 'Y', 'Ỹ': 'Y', 'Ỵ': 'Y',
}

func ConvertToSlug(input string) (string, error) {
	// Kiểm tra ký tự không hợp lệ
	for _, r := range input {
		if !unicode.IsLetter(r) && !unicode.IsNumber(r) && r != ' ' && r != '-' {
			return "", fmt.Errorf("kí tự không hợp lệ: %c", r)
		}
	}

	// Chuyển ký tự tiếng Việt có dấu sang không dấu
	var builder strings.Builder
	for _, r := range input {
		if replacement, exists := vietnameseToASCII[r]; exists {
			builder.WriteRune(replacement)
		} else {
			builder.WriteRune(r)
		}
	}
	normalized := builder.String()

	// Chuyển thành chữ thường và thay khoảng trắng bằng dấu gạch dưới
	lower := strings.ToLower(normalized)
	slug := strings.ReplaceAll(lower, " ", "_")
	return slug, nil
}
